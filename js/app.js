...
function nyTimes() {
  ...
  let apiCall = fetch(`https://api.nytimes.com/svc/topstories/v2/science.json?api-key=${nytimeKey}`);
  apiCall
  .then(res => res.json())
  .then(results => {
...
function TMDb() {
  ...
  .then(results => {
    for(let i = 0; i < results.results.length; i++) {
      fetch("https://api.themoviedb.org/3/movie/" + results.results[i].id + `?api_key=${tmdbKey}`)
      .then(res => res.json())
      .then(resulted => {
